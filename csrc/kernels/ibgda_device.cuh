...
// 8-byte fetch-and-add WQE constructor (mlx5 `MLX5_OPCODE_ATOMIC_FA`, the
// non-masked variant). Layout matches the 4-byte path (ctrl + raddr +
// atomic + data, 4×16B = 64B), but the atomic segment is the standard
// `mlx5_wqe_atomic_seg` (8-byte `swap_add` for the FA operand, `compare`
// reserved/0) and `data_seg.byte_count` is `sizeof(int64_t)`. Used for the
// cumulative head/tail counters where 32-bit wrap is reachable at multi-
// million-step training durations — see `multi-step-correctness.md` (M2).
__device__ static __forceinline__ void ibgda_write_amo_add_wqe_8byte(nvshmemi_ibgda_device_qp_t* qp,
                                                                     const int64_t& value,
                                                                     uint64_t laddr,
                                                                     __be32 lkey,
                                                                     uint64_t raddr,
    ...
    // Standard 8-byte fetch-and-add. No IBGDA_4_BYTE_EXT_AMO_OPMOD modifier.
    ctrl_seg.opmod_idx_opcode = HtoBE32(MLX5_OPCODE_ATOMIC_FA | (wqe_idx << 8));
    atomic_seg.swap_add = HtoBE64(static_cast<uint64_t>(value));
    atomic_seg.compare = 0;

    ctrl_seg.qpn_ds = HtoBE32((qp->qpn << 8) | 4);
    ctrl_seg.fm_ce_se = MLX5_WQE_CTRL_CQ_UPDATE;

    data_seg.byte_count = HtoBE32(sizeof(int64_t));
    data_seg.lkey = lkey;
    data_seg.addr = HtoBE64(laddr);
    ...
    st_na_relaxed(reinterpret_cast<int4*>(ctrl_seg_ptr), *reinterpret_cast<int4*>(&ctrl_seg));
    st_na_relaxed(reinterpret_cast<int4*>(raddr_seg_ptr), *reinterpret_cast<int4*>(&raddr_seg));
    st_na_relaxed(reinterpret_cast<int4*>(atomic_seg_ptr), *reinterpret_cast<int4*>(&atomic_seg));
    st_na_relaxed(reinterpret_cast<int4*>(data_seg_ptr), *reinterpret_cast<int4*>(&data_seg));
}

__device__ __forceinline__ void nvshmemi_ibgda_amo_nonfetch_add_8byte(
    void* rptr, int64_t value, int pe, int qp_id, bool is_local_copy = false) {
    if (is_local_copy) {
        ...
        ibgda_write_amo_add_wqe_8byte(qp, value, reinterpret_cast<uint64_t>(qp->ibuf.buf), qp->ibuf.lkey, raddr, rkey, my_wqe_idx, &wqe_ptrs);
