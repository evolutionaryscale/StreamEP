...
// atomicMax-based writeback for the persistent `reader_prev_*` arrays
// (Option 1, see `multi-step-correctness.md`). The on-the-wire NIC AMO is
// 4-byte (mlx5 ATOMIC_MASKED_FA), so the slot's low 32 bits accumulate
// modulo 2^32. The persistent reader_prev_*[c, peer] is int64, storing the
// FULL cumulative-since-`Buffer`-creation count.
//
// Race-safety: `prev_at_entry` is captured at warp entry, identical across
// all warps within the same iter (writeback only happens at kernel exit, so
// no mid-kernel update races). At writeback, each warp computes
// `new_cumulative = prev_at_entry + (cur_low - prev_at_entry_low) mod 2^32`
// — adding the iter-local advance (always < 2^31 at production) on top of
// the entry-time cumulative gives the correct full count, even after the
// slot's low 32 bits wrap. atomicMax across warps keeps the largest, which
// is monotonic. No wrap detection branch / CAS loop needed: the "high bits"
// naturally accumulate as iters advance.
__forceinline__ __device__ void atomicmax_reader_prev_cumulative(int64_t* prev_addr,
                                                                  int64_t prev_at_entry,
                                                                  uint32_t cur_low) {
    uint32_t advance = cur_low - static_cast<uint32_t>(prev_at_entry);
    int64_t new_cumulative = prev_at_entry + static_cast<int64_t>(advance);
    atomicMax(reinterpret_cast<unsigned long long*>(prev_addr),
              static_cast<unsigned long long>(new_cumulative));
}
...
dispatch_main_kernel(DispatchPoolOut pool_out,
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        int64_t token_idx;
        int64_t prev_rdma_channel_head_at_entry = lane_id < kNumRDMARanks
        ...
        for (token_idx = token_start_idx; token_idx < token_end_idx; ++token_idx) {
            ...
            while (is_token_in_rank_uint64 != 0 and rdma_tail_idx - cached_rdma_channel_head >= env.num_max_rdma_chunked_recv_tokens) {
                cached_rdma_channel_head =
                    static_cast<int>(static_cast<uint32_t>(ld_volatile_global(rdma_channel_head.buffer(lane_id))) - static_cast<uint32_t>(prev_rdma_channel_head_at_entry));
        ...
        if (lane_id < kNumRDMARanks) {
            atomicmax_reader_prev_cumulative(
                ...
                static_cast<uint32_t>(ld_volatile_global(rdma_channel_head.buffer(lane_id))));
        }
        ...
        // C4 force-write (M1): publish (current_seq, 0) to nvl_channel_tail at
        // iter entry, unconditionally. The slot is otherwise written only when
        // tokens flow this iter — under sparse routing a substream can be
        // dormant for ≥ 4096 iters then resume, at which point the 12-bit seq
        // tag in the residual value would falsely match the current iter and
        // the NVL receiver would consume a stale tail. The iter-entry write
        // forces every (channel, dst_nvl) tail slot to carry the current iter's
        // seq before any reader looks. No barrier needed: the receiver gates
        // on prefix_start/end seq-match (written later in this kernel by the
        // meta-poll branch below) before it inspects tail.
        if (elect_one_sync())
            st_relaxed_sys_global(nvl_channel_tail.buffer(),
                                  nvl_pack(tile_signal.dispatch_seq, 0));
        ...
        int64_t prev_rdma_channel_tail_at_entry = lane_id < kNumRDMARanks
                ...
                if (__shfl_sync(0xffffffff, num_tokens_to_recv_from_rdma, src_rdma_rank) > 0) {
                    if (lane_id == src_rdma_rank and cached_rdma_channel_head == cached_rdma_channel_tail)
                        cached_rdma_channel_tail =
                            static_cast<int>(static_cast<uint32_t>(ld_acquire_sys_global(rdma_channel_tail.buffer(src_rdma_rank))) - static_cast<uint32_t>(prev_rdma_channel_tail_at_entry));
        ...
        if (lane_id < kNumRDMARanks) {
            atomicmax_reader_prev_cumulative(
                env.reader_prev_tail + channel_id * kNumRDMARanks + lane_id,
                prev_rdma_channel_tail_at_entry,
        ...
        // C4 force-write (M1): publish (current_seq, 0) to nvl_channel_head at
        // iter entry. Symmetric to the forwarder's nvl_channel_tail force-write
        // — covers the dormant-substream + cross-phase aliasing cases. No
        // barrier needed: the forwarder gates on tail backpressure (cached_head
        // initialized to 0), and only consumes head reads that seq-match the
        // current iter; pre-force-write reads see the prior iter's tag and are
        // ignored by the spin.
        if (elect_one_sync())
...
__device__ int combine_token(bool is_token_in_rank,
    ...
    // Reduce per-(t, k) topk weights — fwd: sum back to topk_weights[t, k]
    // (one source ships the real weight, others ship 0); bwd: sum
    // dL/dweight[t, k] from each source.
    if (lane_id < num_topk) {
        float value = 0;
        #pragma unroll
        for (int i = 0; i < num_topk_ranks; ++i)
...
//             and replaces `< 0` entries with `-last_head - 1`.
//   blocks 2..: reverse-order sentinel encoding of `combined_nvl_head`
...
__global__ void cached_notify_combine_kernel(
    ...
    if (sm_id == 0) {
        ...
        if (warp_id < num_channels) {
            ...
            // SMs 2..(num_channels * 2 - 1) cooperatively cover the
            // dst_rdma_rank dimension; each SM strides by `num_channels * 2 - 2`.
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        while (__any_sync(0xffffffff, num_tokens_to_send > 0)) {
            ...
            for (int i = 0, synced_num_tokens_to_send; i < kNumRDMARanks; ++i) {
                ...
                int processed_tail = ld_acquire_cta(rdma_send_channel_tail + dst_rdma_rank);
        ...
        // C4 force-write (M1): see dispatch_main_kernel forwarder. Fwd
        // dispatch and bwd dispatch_grads share the same dispatch_seq within
        // a layer, so bwd's iter entry must rewrite the slot to overwrite
        // fwd's leftover same-seq value before any bwd reader looks.
        if (elect_one_sync())
            st_relaxed_sys_global(nvl_channel_tail.buffer(),
                                  nvl_pack(tile_signal.dispatch_seq, 0));
        ...
        // C4 force-write (M1): see dispatch_main_kernel NVL receiver. Same
        // rationale as bwd forwarder above — fwd dispatch and bwd
        // dispatch_grads share dispatch_seq within a layer.
        if (elect_one_sync())
            st_relaxed_sys_global(nvl_channel_head.buffer(),
                                  nvl_pack(tile_signal.dispatch_seq, 0));
    ...
    if (warp_role == WarpRole::kNVLSender) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (clock64() - start_time > NUM_TIMEOUT_CYCLES and lane_id < kNumRDMARanks) {
                    printf("DeepEP combine NVL sender timeout, channel: %d, RDMA: %d, nvl: %d, dst NVL: %d, RDMA lane: %d, head: %d, tail: "
                           "%d, start: %d, end: %d\n",
            ...
            for (int i = 0; i < kNumRDMARanks; ++i) {
                ...
                for (int chunk_idx = 0; chunk_idx < num_tokens_in_chunk; ++chunk_idx, ++token_idx) {
                    ...
                    // the per-slot weight (fwd: pool_topk_weight = topk_weights[t, k]
                    // for the source t that landed in this slot; bwd: weight_grads).
                    // For non-local k (slot == -1) we ship 0 — receiver's K-way sum
                    // then yields the correct (t, k) value since exactly one sender
                    // has the non-zero contribution.
                    if (lane_id < num_topk) {
        ...
        if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
            ...
            int64_t prev_rdma_channel_head_at_entry =
            ...
            for (int token_start_idx = 0; token_start_idx < num_tokens_to_combine; token_start_idx += num_max_rdma_chunked_send_tokens) {
                ...
                while (sub_warp_id == 0 and lane_id == 0) {
                    int cur_iter_head = static_cast<int>(static_cast<uint32_t>(ld_volatile_global(rdma_channel_head.buffer(dst_rdma_rank))) - static_cast<uint32_t>(prev_rdma_channel_head_at_entry));
                ...
                for (int token_idx = token_start_idx + sub_warp_id; token_idx < token_end_idx; token_idx += kNumWarpsPerForwarder) {
                    ...
                    combine_token<NUM_MAX_NVL_PEERS, dtype_t, NUM_MAX_NVL_PEERS, true, kNumStages, kNumTMALoadBytes>(
                        expected_head >= 0,
                        expected_head,
                        lane_id,
                        hidden_int4,
            ...
            if (lane_id == 0) {
                atomicmax_reader_prev_cumulative(
                    combine_reader_prev_head + channel_id * kNumRDMARanks + dst_rdma_rank,
                    prev_rdma_channel_head_at_entry,
                    static_cast<uint32_t>(ld_volatile_global(rdma_channel_head.buffer(dst_rdma_rank))));
            }
            ...
            for (int64_t token_idx = token_start_idx + warp_id; token_idx < token_end_idx; token_idx += kNumRDMAReceivers) {
                ...
                while (cached_channel_tail_idx <= expected_head) {
                    cached_channel_tail_idx =
                        static_cast<int>(static_cast<uint32_t>(ld_acquire_sys_global(rdma_channel_tail.buffer(lane_id))) - static_cast<uint32_t>(prev_rdma_channel_tail_at_entry));
                    if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
                ...
                combine_token<kNumRDMARanks, dtype_t, kNumTopkRDMARanks, false, 2>(
...
#define COMBINE_LAUNCH_CASE(num_rdma_ranks)                                          \
    {                                                                                \
        ...
        LAUNCH_KERNEL(&cfg, kernel,                                                  \
                      ...
                      recv_rdma_rank_prefix_sum,                                     \
                      gbl_channel_prefix_matrix,                                     \
                      compute_done_per_token, combine_seq,                           \
